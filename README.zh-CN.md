...
- 提升自动刷新可靠性，避免刷新任务卡住后菜单栏 `Refresh` 失效。
- 菜单栏 `Refresh` 改为强制刷新，并只采用最新一次请求结果。
- 优化 Codex app-server 子进程读取方式，减少 stdout/stderr 阻塞风险。
- 额度窗口到期时显示 `now`，并主动触发一次新数据刷新。
- 减少 Touch Bar 右侧常驻图标的重复注册，降低图标消失或抖动概率。
